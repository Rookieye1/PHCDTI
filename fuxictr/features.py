...
from PHCDTI.FuxiCTR.fuxictr.preprocess import Tokenizer, Normalizer
...
class FeatureEncoder(object):
    ...
    def read_csv(self, data_path):
        ...
        # print(self.feature_cols, self.label_col)
        all_cols = self.feature_cols + [self.label_col] # [self.label_col]变为self.feature_col
    ...
    def preprocess(self, ddf, fill_na=True):
        ...
        all_cols = [self.label_col] + self.feature_cols[::-1] # [self.label_col]变为self.label_col
        for col in all_cols:
            name = col["name"]
        ...
        active_cols = [self.label_col["name"]] + [col["name"] for col in self.feature_cols if col["active"]] # [self.label_col["name"]]变为self.label_col["name"]
