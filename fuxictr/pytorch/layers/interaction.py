...
class InnerProductLayer(nn.Module):
    ...
    def __init__(self, num_fields=None, output="product_sum_pooling"):
        ...
        if num_fields is None:
            ...
            #torch.triu(torch.ones(num_fields, num_fields), 1)创建了一个由1构成的num_fields行num_fields列的张量，
            # 然后使用torch.triu函数将其转换为一个上三角矩阵。1表示保留上三角部分，0表示下三角部分。
    ...
    def forward(self, feature_emb):
        if self._output_type in ["product_sum_pooling", "Bi_interaction_pooling"]: 
            ...
            flat_upper_triange = torch.masked_select(inner_product_matrix, self.upper_triange_mask.bool())
    ...
    def __init__(self, num_fields=None, output="product_sum_pooling"):
        ...
        if num_fields is None:
            ...
            self.upper_triange_mask = nn.Parameter(torch.triu(torch.ones(num_fields, num_fields), 1).type(torch.ByteTensor),
                                                   requires_grad=False)
        # 初始化两个MLP层
        self.mlp_u = MLP(embedding_dim, hidden_dim, embedding_dim)
        self.mlp_q = MLP(embedding_dim, hidden_dim, embedding_dim)
